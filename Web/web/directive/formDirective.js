...
define(['app', 'bpmn'], function (app, Bpmn) {
    ...
    //             if($attrs.time!=null && $attrs.time!=undefined){
    //                 if($attrs.time=='Y-m'){
    //                     format=$attrs.time;
    //                 }
    //             }
        ...
        return {
            ...
            link: function ($scope, $elem, $attrs, ngModel) {
                ...
                if ($attrs.min != undefined) {
                    var numberdaysMin = new Date(new Date().setDate(new Date().getDate() + Number($attrs.min)));
                } else {
                    var numberdaysMin = false;
                }
                if ($attrs.max != undefined) {
                    var numberdaysMax = new Date(new Date().setDate(new Date().getDate() + Number($attrs.max)));
                } else {
                    var numberdaysMax = false;
                }
                ...
                $elem.datetimepicker({
                    ...
                    closeOnDateSelect: true,
                    validateOnBlur: true,
                    minDate: numberdaysMin || false,
                    maxDate: numberdaysMax || false,
                    onChangeDateTime: function (ct, $i) {
                        $scope.$apply(function () {
                            if (ct != null) {
                                ngModel.$setViewValue(ct.dateFormat(format));
    ...
    app.directive('showChecker', ['$resource', 'EngineApi', 'Auth', function ($resource, EngineApi) {
        return {
            ...
            link: function (scope, element, attrs) {
                ...
                scope.ProcessRole = attrs.processRole!=null &&  attrs.processRole!= undefined  ? '('+attrs.processRole +')' : '';
            },
