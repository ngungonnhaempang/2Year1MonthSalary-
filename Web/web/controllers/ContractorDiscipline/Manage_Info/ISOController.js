define(['myapp', 'angular'], function (myapp) {
    myapp.controller('ISOController', ['ConDisciplineService', '$scope', '$filter', '$compile', '$routeParams', '$resource', '$location', 'i18nService', 'Notifications', 'User', 'Forms', 'Auth', 'uiGridConstants', '$http', 'EngineApi', '$upload', '$translatePartialLoader', '$translate', 'GateGuest', '$timeout',
        function (ConDisciplineService, $scope, $filter, $compile, $routeParams, $resource, $location, i18nService, Notifications, User, Forms, Auth, uiGridConstants, $http, EngineApi, $upload, $translatePartialLoader, $translate, GateGuest, $timeout) {
                    ...
                    action: function () {
                        ...
                        if (resultRows.length == 1) {
                            if (resultRows[0].Status == 'UAP') {
                                ...
                                params = resultRows[0];
                                params.Status = 'X';
                                ConDisciplineService.SaveISO_Application().save({}, params).$promise.then(function (res) {
                                    console.log(res);
                                    $scope.searchISO();
                                    $timeout(function () {
                            ...
                            if (resultRows[0].Status == 'A') {
                                $scope.checkCode(resultRows[0], function (_hasUpgrade) {
                                    if (!_hasUpgrade) {
                                        ...
                                        $scope.data.ISO_AppCode = $scope.data.AppCode + '-0' + $scope.data.Version
            ...
            $scope.saveISO = function () {
                ...
                params.AppCode = $scope.data.AppCode
                params.AppName = $scope.data.AppName
