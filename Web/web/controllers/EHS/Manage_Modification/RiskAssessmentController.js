define(['myapp', 'angular'], function (myapp) {
    myapp.controller('RiskAssessmentController', ['ModificationService', '$scope', '$filter', '$compile', '$routeParams', '$resource', '$location', 'i18nService', 'Notifications', 'User', 'Forms', 'Auth', 'uiGridConstants', '$http', 'EngineApi', '$upload', '$translatePartialLoader', '$translate', 'GateGuest', '$timeout',
        function (ModificationService, $scope, $filter, $compile, $routeParams, $resource, $location, i18nService, Notifications, User, Forms, Auth, uiGridConstants, $http, EngineApi, $upload, $translatePartialLoader, $translate, GateGuest, $timeout) {
            ...
            $scope.searchRiskAssessment = function () {
                ModificationService.SearchRiskAssessment().get({
                    Lang: lang,
            ...
            var colRisk = [
                {
                    ...
                    minWidth: 100,
                    cellTemplate: "<a ng-click='grid.appScope.getProcessRiskLog(row)' style='padding:5px;display:block; cursor:pointer'>{{COL_FIELD}}</a>",
            ...
            var gridMenu = [{
                ...
                action: function () {
                    $location.url('/Manage_Modification/RiskAssessment/Create/null');
                },
                order: 1
            }, {
                ...
                action: function () {
                    ...
                    if (resultRows.length == 1) {
                        if (resultRows[0].Status == "N") {
                            $location.url("/Manage_Modification/RiskAssessment/Update/" + resultRows[0].Risk_AppID);
                        }
                        else {
                ...
                action: function () {
                    ...
                    if (resultRows.length == 1) {
                        if(resultRows[0].Status!="X"){
                            var href = '#/Manage_Modification/RiskAssessment/Print/' + resultRows[0].Risk_AppID
                            window.open(href);
                        }else{
            ...
            },
            {
                ...
                action: function () {
                    var resultRows = $scope.gridApi.selection.getSelectedRows();
                    if (resultRows.length == 1) {
                        if (resultRows[0].Status == 'Q' || resultRows[0].Status == 'E') {
                            debugger
                            ModificationService.CheckForeignKey().get({
                                Table: 'EvaluationApplication',
                                AppID: resultRows[0].Risk_AppID
                            }).$promise.then(function (res) {
                                debugger
                                if(!res.Evaluation_AppID && !res.Closing_AppID){
                                    $scope.itemDelete = {
                                        Name: resultRows[0].MD_ProjectID,
                                        ID: resultRows[0].Risk_AppID,
                                        Status:"SPD",
            ...
            $scope.getProcessRiskLog = function (obj) {
                debugger
                var _VoucherID = obj.entity.Risk_AppID;
