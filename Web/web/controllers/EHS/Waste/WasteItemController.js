...
define(['myapp', 'controllers/EHS/Waste/Directive/WasteItemDirective', 'angular'], function (myapp, angular) {
    myapp.controller('WasteItemController', ['$filter', '$http',
        ...
        function ($filter, $http, $routeParams,
            ...
            var lang = window.localStorage.lang||'EN'; //language
            ...
            $q.all([loadMethodName(), loadCompany(),loadValuation(),loadRoots(),loadWasteType(),loadWasteState(),loadArea(),loadUnitPrice()]);
            /**
            ...
            function loadCompany() {
                ...
                CompanyService.GetInfoBasic.GetOwnerCompany({}).$promise.then(function (data) {
            ...
            function loadValuation(){
                ...
                CompanyService.GetInfoBasic.GetValuationbyLang({Lang: lang}).$promise.then(function (res) {
                    $scope.ValuationData= res;
            ...
            function loadUnitPrice() {
                var deffered = $q.defer();
                WasteItemService.GetInfoBasic.GeUnitPricebyLang({Lang: lang}).$promise.then(function (res) {
                    $scope.UnitPrince = res;
                    deffered.resolve(res);
            ...
            function  loadWasteType() {
                var deffered = $q.defer();
                WasteItemService.GetInfoBasic.GetWasteTypebyLang({Lang: lang}).$promise.then(function (res) {
            ...
            function  loadWasteState() {
                var deffered =$q.defer();
                WasteItemService.GetInfoBasic.GetWasteStatebyLang({Lang: lang}).$promise.then(function (res) {
                    $scope.WasteStateData = res;
                    deffered.resolve(res);
                },function (err) {
                    deffered.error(err);
                })
            ...
            function loadWasteItemDetails(WasteID) {
                ...
                WasteItemService.FindByID({
                    ...
                    $scope.recod.waste_id = data.data.WasteID;
                    $scope.recod.waste_originid = data.data.WasteOriginID;
                    $scope.recod.ItemCode = data.data.ItemCode;
                    $scope.recod.state = data.data.State;
                    $scope.recod.description_CN = data.data.Description_TW;
                    $scope.recod.description_VN = data.data.Description_VN;
                    $scope.recod.method_id = data.data.MethodID;
                    $scope.recod.comp_originid = data.data.CompOriginID;
                    $scope.recod.Status = data.data.Status;
                    $scope.recod.IsValuation= data.data.WasteValue;
                    $scope.recod.WasteType= data.data.WasteType;
                    $scope.recod.Roots= data.data.Source;
                    $scope.recod.Area= data.data.Area;
                    $scope.recod.NameCN =  data.data.Description_CN
                    $scope.recod.NameVN =data.data.Description_EN ;
                    $scope.priceHistory.Price= data.data.UnitPrice;
                    $scope.priceHistory.UnitPrice= data.price.UnitPrice;
                    $scope.recod.WasteID= data.data.WasteID;
                    $scope.priceHistory.StartDate=$filter('date')( data.price.StartDate,'yyyy-MM-dd');
                    $scope.priceHistory.EndDate= $filter('date')( data.price.EndDate,'yyyy-MM-dd');
