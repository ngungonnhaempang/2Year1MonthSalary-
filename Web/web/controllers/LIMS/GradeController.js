        ...
        function ($scope, $filter, $compile, $routeParams, $resource, $location, i18nService, Notifications, Auth, uiGridConstants,
                    ...
                    if (res.length > 0) {
                        ...
                        if (query.status == 'P' || query.status == 'S') {
                            $scope.HisttoryData = {
                                ...
                                'lotNo': $scope.note.Material,
                                'grades': res[0].Grades,
                                'Type':   $scope.CategoryList.filter((el) => {
                                    return (el.TypeID === $scope.TypeID);
                                     }),
                                'Lang':$scope.lang,
                                'ID':$scope.ID
...

            {
                ...
                order: 8
                    ...
                    title: $translate.instant('Special_modify'),
                    action: function () {
                        console.log($scope.username);
                        var params = {
                            UserID: $scope.username
