        ...
        function ($filter, $http, $routeParams,
            ...
            return {
                ...
                controller: function ($scope) {
                    ...
                    //support only one user can modify grade version voucher status is publish and dont increase version
                    function saveSpecial_modifyversion(callback) {
                        var mar = paraData('');
                        console.log(mar);
                        LIMSService.gradeVersion.Special_ModifyGradeVersion(mar).$promise.then(function (res) {
                            if (res.Error) {
                                callback('', res.Error);
                            } else {
                                callback(res);
                ...
                $scope.fnSaveStatus=()=>
                    ...
                    $scope.submit_special_modify = function () {
                        if ($scope.checkList.length <= 0) {
                            Notifications.addError({
                                'status': 'error',
                                'message': 'Don\'t get leader'
                        ...
                        formVariables.push({name: 'ChecherArray', value: $scope.checkList});
                        saveSpecial_modifyversion(function (result, err) {
                            if (!result) {
                                Notifications.addError({'status': 'error', 'message': err});
