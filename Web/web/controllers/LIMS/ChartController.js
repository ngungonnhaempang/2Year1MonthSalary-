define(['myapp', 'angular'], function (myapp, angular) {
    myapp.controller('ChartController', ['$scope', '$filter', '$http', '$q', '$compile', '$routeParams', '$resource', '$location', 'i18nService',
        ...
        function ($scope, $filter, $http, $q, $compile, $routeParams, $resource, $location, i18nService, Notifications, Auth,
            ...
            function SingleChart(chartname, chartData) {
                var obj = $(chartname);
                var options = {
                    chart: {
                        renderTo: 'container',
            ...
            function SingleChartData(value, chartname) {
                ...
                LIMSService.Entrusted.SingleTrendChart({}, value).$promise.then(function (res) {
                    if (res.Error) {
                        ...
                        var chartdata = res;
                        //ShowChart(  ,res)
                        SingleChart(chartname, chartdata);
                    ...
                    if (res.Error) {
                        ...
                        var options = {
                            ...
                            tooltip: {
                                formatter: function () {
                                    var no=this.x;
                                    var ll = chartdata.ChartCreater.MainData[no].ProdDate;
                                    console.log(ll);
                                    return '<b>'+ll +' :'+ this.x + '</b><br/>' +this.y+
                                 '<br/>'
