...
module.exports = function (app, request, config, express) {
    ...
    app.get('/api/cmis/showfileHSE', function (req, res) {

        if (req.query.filename) {
            fs.exists('D:/QCFiles/' + req.query.filename, function (exists) {
                if (exists) {
                    var mimetype = mime.lookup('D:/QCFiles/' + req.query.filename);
                    if (mimetype.includes('image/') || mimetype.includes('/pdf')) {
                        res.setHeader('Content-Type', mimetype)
                        fs.createReadStream(path.join('D:/QCFiles/', req.query.filename.trim())).pipe(res).on('error', function (e) {
                            res.send(500, e);
                        });
                    } else {
                        var newFileName = encodeURIComponent(req.query.filename);
                        res.setHeader('Content-Disposition', 'attachment;filename*=UTF-8\'\'' + newFileName);
                        var file = 'D:/QCFiles/' + req.query.filename;
    ...
    app.get('/api/cmis/downContractorFile', function (req, res) {
        console.log(req.query.filename)
        if (req.query.filename) {
            var filepath = path.join('D:/ContractorFile/', req.query.filename)
            res.download(filepath, req.query.filename);

        }
    })

    app.get('/api/cmis/imgUrl', function (req, res) {
        console.log(req.query.filename)
        if (req.query.filename) {
           var filepath = path.join('D:/ContractorFile/', req.query.filename)
           var url = fs.readFileSync(filepath, {encoding: 'base64'});
